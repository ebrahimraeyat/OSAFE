...
def make_rectangular_slab(
    ...
    plan, _, _ = osafe_funcs.get_left_right_offset_wire_and_shape(obj.Base.Shape, obj.left_width, obj.right_width)
    points = osafe_funcs.get_sort_points(plan.Edges)
...
class RectangularSlab(ArchComponent.Component):
    ...
    def execute(self, obj):
        ...
        if obj.Auto_Plan and obj.Base:
            plan, _, _ = osafe_funcs.get_left_right_offset_wire_and_shape(obj.Base.Shape, sl, sr)
            points = osafe_funcs.get_sort_points(plan.Edges, get_last=True)
...
class ViewProviderRectangularSlab:
    ...
    def claimChildren(self):
        ...
        if self.Object.plan:
            children.append(self.Object.plan)
        return children

    def getIcon(self):
        return str(Path(__file__).parent.parent / "osafe_images" / "rectangular_slab.svg")
