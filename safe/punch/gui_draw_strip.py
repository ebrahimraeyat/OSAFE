...
class DrawStrip(gui_lines.Line):
    ...
    def GetResources(self):
        ...
        tool_tip = QtCore.QT_TRANSLATE_NOOP(
            ...
            "Draw Strip")
        path = str(
                   Path(__file__).parent.absolute() / "Resources" / "icons" / "draw_strip.svg"
                   )
        return {'Pixmap': path,
                'MenuText': menu_text,
                'ToolTip': tool_tip}
    ...
    def Activated(self):
        ...
        if selections:
            ...
            if hasattr(wire, 'Proxy') and hasattr(wire.Proxy, 'Type')  and wire.Proxy.Type == 'Wire':
                ...
                shape = punch_funcs.get_left_right_offset_wire_and_shape(wire, self.strip_left_width, self.strip_right_width)[0]
    ...
    def taskbox(self):
        ...
        w = Gui.PySideUic.loadUi(str(punch_path / 'Resources' / 'ui' / 'draw_strip.ui'))
